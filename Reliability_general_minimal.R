...
library(mgc)
library(doParallel)  
source('/Users/leiai/projects/Scripts_HUB/Reliability_Discriminability/trace_only_unba.R')
...
run_icc <- function(a1,a2,outfolder){
  ...
  #print('Load in the first datset')
  for(i in 1:subn){
    if (file.names1[i]=='sub-0025430a.1D'){
      continue
    }
  ...
  # handel ccs reorder
  #if (a1 == 'ccs'){
   # xcp_data=xcp_data[ccsOrder,]
  #}
  #if (a2 == 'ccs'){
  #  cpac_data=cpac_data[ccsOrder,]
  #}
  ...
  #discr=discr.stat(as.matrix(rbind(xcp_data,cpac_data)),c(rep(1,subn),rep(2,subn))
...
iccplot_old <- function(data,outfile){
  ...
  #Yeomaplab[19] = 'Visual';
  #Yeomaplab[48] = 'Somatomotor';
  #Yeomaplab[69] = 'DorsalAttention';
  #Yeomaplab[90] = 'VentralAttention';
  #Yeomaplab[108] = 'Limbic';
  #Yeomaplab[126] = 'FrontoParietal';
  #Yeomaplab[158] = 'Default';
  #Yeomaplab[190] = 'No overlap';
  ...
  heatmap.2(data,
            ...
            sepcolor="white",
            #main = paste0(condition_list[i],'_30min'),
            labRow = Yeomaplab,
            labCol  = Yeomaplab,
            adjCol = c(NA,0.4),
            cexRow = 0.7,
...
#basefolder='~/projects/CPAC_fmriprep/Reliability_discriminibility/Finalizing/Minimal'
basefolder='~/Documents/projects/CPAC_fmriprep/Reliability_discriminibility/Finalizing/Minimal'

num_roi=200
num_edge=num_roi*num_roi/2-num_roi/2

setwd(paste0(basefolder,'/ROI/ROI_Schaefer',num_roi))
outfolder=paste0(basefolder,'/figures/ICC_Schaefer',num_roi)
...
runs=c('cpac_default_off_nuisance','fmriprep_1','Upenn_cpacfmriprep','ABCD','dpabi','ccs')
#runs=c('dpabi','ABCD','cpac_default_off_nuisance','fmriprep_1','Upenn_cpacfmriprep','cpac_fmriprep')
#runs=c('ccs','dpabi')
runs=c('cpac_fmriprep','cpac_default_off_nuisance','fmriprep_1','Upenn_cpacfmriprep','ABCD','dpabi','ccs')

runs=c('fmriprep_MNI2004_2mm','cpac_fmriprep','cpac_default_off_nuisance','fmriprep_1','ABCD','dpabi','ccs')

runs=c('cpac_FX_no_filt','cpac_fmriprep_with_filt','cpac_fmriprep','cpac_3dvolreg_36')

#####################################################################
#Figure1. ICC and Discriminability
runs=c('cpac_fmriprep_MNI2004_2mm','cpac_default_off_nuisance','abcd','fmriprep_2','dpabi','ccs','fmriprep_MNI2004_2mm','cpac_fmriprep')
#runs=c('ccs','fmriprep_MNI2004_2mm')
runs=c('cpac_fmriprep_MNI2004_2mm','ABCD')

runs=c('cpac_default_off_nuisance','dpabi')
#runs=c('cpac_fmriprep_MNI2004_2mm','fmriprep_MNI2004_2mm')


runs=c('fmriprep_MNI2004_2mm','cpac_fmriprep_MNI2004_2mm','cpac_default_off_nuisance','cpac_fmriprep_MNI2004_2mm_anat_mask','cpac_fmriprep_MNI2004_2mm_func_mask','cpac_fmriprep_MNI2004_2mm_coreg','cpac_fmriprep_MNI2004_2mm_N4','cpac_fmriprep_MNI2004_2mm_no_stc')
runs=c('fmriprep_2','cpac_fmriprep','cpac_default_off_nuisance','cpac_fmriprep_anat_mask','cpac_fmriprep_func_mask','cpac_fmriprep_coreg','cpac_fmriprep_N4','cpac_fmriprep_stc')

#runs=c('cpac_fmriprep_MNI2004_2mm','fmriprep_MNI2004_2mm','cpac_default_off_nuisance')

runs=c('CPAC_ABCD_scratch_0309_final','cpac_default_off_nuisance','ABCD')

#icc_data=matrix(, nrow = 15, ncol = num_edge);
